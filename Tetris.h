...
void addScore(int newScore) {
  ...
  //direction = waitForDirection(DOWN | RIGHT_DOWN | LEFT_DOWN)
  // if (tetromino == NULL)
  //    tetromino = spawnFirstTetromino();
  // else
  //    moveTetromino(direction)
      //result = checkCollision();
      //if (result == TRUE)
      //    moveTetromino(reverse(direction))
      //    lines[] = checkForLines()
      //    deleteLinesAndShift()
      //    addScore(lines.count)
      //    canSpawn = canSpawnTetromino()
      //    if (canSpawn)
      //      changeTetromino()
      //    else
      //      finishGame()
...
/*int move_shape_left(Tetro* tetro) {

  if (tetro == NULL) {
    Serial.print("\nCannot move left NULL tetro\n");
    return;
  }
  int currY = tetro->corner_y;
  if (currY <= 0) {
    Serial.print("\nCannot move left anymore\n");
    return;
  }

  tetro->corner_y = currY - 1;
  }

  int move_shape_right(Tetro* tetro) {

  if (tetro == NULL) {
    Serial.print("\nCannot move right NULL tetro\n");
    return;
  }
  int currY = tetro->corner_y;
  if (currY >= WIDTH - 1) {
...
/*int move_shape_down(Tetro* tetro, char** Board) {

  if (tetro == NULL) {
    Serial.print("\nCannot move down NULL tetro\n");
    return;
  }

  int upperLeftCorner_x = tetro->corner_x;
  int upperLeftCorner_y = tetro->corner_y;
  int bottomLeftCorner_x = tetro->corner_x + tetro->shape_dim;
  int bottomLeftCorner_y = tetro->corner_y;
  int bottomRightCorner_x = tetro->corner_x + tetro->shape_dim;
  int bottomRightCorner_y = tetro->corner_y + tetro->shape_dim;

  int hasCollision = 0;
  for (int i = bottomLeftCorner_y; i <= bottomRightCorner_y; i++) {
    int justDown_y = i;
    int justDown_x = bottomLeftCorner_x + 1;
    if (justDown_y < 0) {
      Serial.print("\nTetro touched the bottom\n");
      hasCollision = 1;
      break;
    }
  }
